...
reconhecimentoFacil = dlib.face_recognition_model_v1("recursos/dlib_face_recognition_resnet_model_v1.dat")
indices = np.load("recursos/indices_rn.pickle", allow_pickle=True)
descritoresFaciaias = np.load("recursos/descritores_rn.npy")
limiar = 0.5
...
for arquivo in glob.glob(os.path.join("fotos", "*.jpg")):
    ...
    for face in facesDetectadas:
        ...
        descritorFacial = reconhecimentoFacil.compute_face_descriptor(imagem, pontosFaciais)
        listaDescritorFacial = [fd for fd in descritorFacial]
        npArrayDescritorFacial = np.asarray(listaDescritorFacial, dtype=np.float64)
        npArrayDescritorFacial = npArrayDescritorFacial[np.newaxis, :]

        distancias = np.linalg.norm(npArrayDescritorFacial - descritoresFaciaias, axis=1)
